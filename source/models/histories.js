...
exports.getHistories = (fin, cb) => {
  db.query(`SELECT h.id, h.userId, v.image, v.name AS vehicleName, h.qty, h.rentStartDate, h.rentEndDate, h.isReturned  FROM histories h LEFT JOIN users u ON h.userId = u.id LEFT JOIN vehicles v ON h.vehicleId = V.id WHERE u.name LIKE '%${fin.userName}%' AND v.name LIKE '%${fin.vehicleName}%' LIMIT ${fin.limit} OFFSET ${fin.offset}`, (err, res) => {
    if (err) throw err;
    cb(res);
  });
};
...
exports.getHistory = (id, cb) => {
  db.query('SELECT h.id, h.userId, v.image, v.name AS vehicleName, h.qty, h.rentStartDate, h.rentEndDate, h.prepayment, h.methodPayment, h.isReturned  FROM histories h LEFT JOIN users u ON h.userId = u.id LEFT JOIN vehicles v ON h.vehicleId = V.id WHERE h.userId=?', [id], (err, res) => {
    if (err) throw err;
    cb(res);
  });
};
